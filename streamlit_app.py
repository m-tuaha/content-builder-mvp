...
if generate_btn and prompt:
    ...
    # SYSTEM PROMPT: Paste your finalized prompt below (not shown for brevity)
...
if st.session_state.last_output:
    ...
    if channel == "whatsapp":
        st.markdown("#### Buttons")
        # Buttons are a list of dicts, show editable fields for each
        buttons = output.get("buttons", [])
        if not buttons:
            buttons = [{} for _ in range(2)]  # Show at least 2 empty rows
        new_buttons = []
